...
def generate_topic(state: State):
    """농담 주제를 생성하는 LLM 호출"""
    msg = model.invoke("주제도 모르는 녀석을 주제로 재미있는 농담 주제를 하나 짧게 알려줘 without emojis")
...
print("\n\n ## 초기 실행 결과 ##")
print(f"**생성된 주제: {' '.join(str(state['topic']).split())[:10]}")
print(f"**작성된 농담: {' '.join(str(state['joke']).split())[:10]}")
print(f"**농담 평가: {' '.join(str(state['evaluation']).split())[:10]}")
...
# 상태 업데이트 (주제를 닭으로 변경)
new_config = graph.update_state(selected_state.config, values={"topic": "식물원"}) #새로운 설정을 적용
print(f"\n새로운 설정: {new_config}")
