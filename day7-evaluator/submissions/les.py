...
class Feedback(BaseModel):
    ...
    grade: Literal["funny", "not funny", "creative"] = Field(
        description="Decide if the joke is funny or not. creative is when the joke is not funny but it is creative. If the joke is creative, you don't need to provide feedback.",
    )
    feedback: str = Field(
...
def llm_call_generator(state: State):
    ...
    if state.get("feedback"):
        ...
        msg = llm.invoke(f"Write a random story about {state['topic']}")
...
def llm_call_evaluator(state: State):
    ...
    return {"funny_or_not_or_creative": grade.grade, "feedback": grade.feedback}
...
def route_joke(state: State):
    ...
    if state["funny_or_not_or_creative"] == "funny":
        print("[END] 웃긴 농담으로 판정되어 종료")

        return "Accepted"
    elif state["funny_or_not_or_creative"] == "not funny":
    ...
    elif state["funny_or_not_or_creative"] == "creative":
        if state.get("attempts", 0) >= 3:
            print("[END] 3회 개선 시도 도달 → 강제 종료")
            return "Accepted"
        print("[LOOP] 개선 피드백 반영하여 재시도")    
