#기본 설정
#병렬화와 비슷해보이지만 차이점이라하면 병렬화는 명확하게 내가 이해하고 노드를 만들어서 활용하고
#오케스트레이터는 계획을 만들고 워커를 동적으로 만들어서 일 하게 하고 결과를 합치는 방식이다.
...
# Invoke
# state = orchestrator_worker.invoke({"topic": "LLM 스케일링 법칙에 관한 보고서 짧게 작성"})
state = orchestrator_worker.invoke({"topic": "초보자를 위한 Python 입문 가이드"})
