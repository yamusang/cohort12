...
# 기본 llm invoke
print(f"기본 llm invoke \n {llm.invoke('데이터셋에 대한 정보들을 신뢰할만 한가? 정확도가 얼마나 되나?')}")
print()
print(f"기본 llm invoke \n {llm.invoke('2에 3을 나누면?')}")
...
#Invoke
output = structured_llm.invoke("데이터셋에 대한 정보들을 신뢰할만 한가? 정확도가 얼마나 되나?")
print(f"\n1. 구조화된 출력을 위한 증강 \n {output}")
...
#Invoke
msg = llm_with_tools.invoke("2에 3을 나누면?")
print(f"\n2. 도구를 위한 증강 \n {msg}")
